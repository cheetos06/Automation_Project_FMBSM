...
def resolve_endpoint(
    ...
    *,
    ...
    """Resolve the public API endpoint without depending on the AWS host.

    Only the endpoint is remotely configurable. Credentials and the pinned
    encryption certificate always remain in the local private configuration.
    A last-known-good manifest is cached atomically so a GitHub outage does not
    ...
    try:
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "FMBSM-Token-Pool-Client/endpoint-discovery",
            },
        )
...
def _write_manifest(path: Path, value: dict[str, Any]) -> None:
    try:
        ...
        # Endpoint discovery must never make an otherwise valid local config
        # unusable merely because the cache directory is read-only.
        return
